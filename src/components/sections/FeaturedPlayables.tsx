        ...
        <AnimatedSection delay={50}>
          ...
          <div className="flex flex-wrap justify-center gap-6">
            {featuredPlayables.map((game, i) => (
              ...
              >
                <div ref={el => { cardRefs.current[i] = el; }} className="h-full">
...
function PlayableCard({
  ...
  return (
    <div
      className={`group relative flex flex-col h-full rounded-2xl overflow-hidden border border-slate-200/60 dark:border-white/8 bg-white/60 dark:bg-white/[0.03] backdrop-blur-sm transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl ${game.glowColor} ${game.borderGlow} dark:${game.borderGlow}`}
    >
      {/* ── Cover ── */}
      <div
      ...
      <div className="flex flex-col flex-1 p-5">
        ...
        <div className="mb-4">
          <div className="relative">
            <select
              value={activeTask}
              onChange={(e) => setActiveTask(Number(e.target.value))}
              className="w-full appearance-none py-2 pl-3 pr-8 rounded-lg text-xs font-bold tracking-wide bg-slate-100 dark:bg-white/6 border border-slate-200/80 dark:border-white/10 text-slate-700 dark:text-slate-300 cursor-pointer focus:outline-none focus:ring-2 focus:ring-indigo-500/40 transition-colors"
            >
              {game.tasks.map((task, idx) => (
                <option key={task.name} value={idx} className="bg-white dark:bg-[#1a1f2e]">
                  {task.name}
                </option>
              ))}
            </select>
            <svg className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 dark:text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
